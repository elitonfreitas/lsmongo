...
const query_match_1 = require("./utils/query-match");
const sort_compare_1 = require("./utils/sort-compare");
class Collection {
    constructor(db, name, opts) {
        opts = opts || {};
        this.name = name;
        this.storage = db.storage || new mock_storage_1.default();
    ...
    find(query, opts) {
        ...
        let data;
        let ids = is_object_1.default(query) ? null : (query instanceof Array ? query : [query]);
        let filterOpts = {
        ...
        if (ids) {
            ...
            data = this._filter((key, val) => {
                return query_match_1.default(query, val);
            }, filterOpts);
        }
        else {
        ...
        if (opts.sort) {
            data.sort((a, b) => {
                return sort_compare_1.default(opts.sort, a, b);
            });
        }
        if (opts.limit) {
    ...
    findOne(query, opts) {
        ...
        opts = opts || {};
        let data;
        let id = is_object_1.default(query) ? null : query;
        ...
        if (id) {
            ...
            if (data && quickTarget && !query_match_1.default(query, data)) {
                return null;
            }
        }
    ...
    update(query, values, opts) {
        ...
        if (!values || !is_object_1.default(values)) {
